...
- 当前预览版：`v0.4.2-preview.1`
- 当前目标：M4 自部署模板扩展（通用 deploy + MinIO/ELK/PowerJob）
- 设计约束：`v0.4.x` 进入冻结期，冻结规则见 `docs/product-design/10-v0.4.x设计冻结说明.md`
...
- `v0.4.x` 设计冻结已生效：允许模板与插件扩展，不允许改动阶段与执行语义
...
- 模板交付能力以 DELIVERY_GATE 为准。
- `v0.4.x` 冻结期内，如需突破冻结边界，必须先走变更申请并获得确认。
